...
fn dupes_only_emits_nul_separated() {
    ...
    let out = fifi().arg("--dupes-only").arg(root).assert().success();
...
fn json_output_has_expected_shape() {
    ...
    let out = fifi().arg("--json").arg(root).assert().success();
    let stdout = String::from_utf8(out.get_output().stdout.clone()).unwrap();
...
fn json_includes_unique_only_when_flagged() {
    ...
    // No duplicates means exit code 1 — orthogonal to what we're testing.
    // Just inspect stdout.
...
#[test]
fn exit_code_zero_when_duplicates_found() {
    let td = TempDir::new().unwrap();
    let root = td.path();
...
#[test]
fn exit_code_one_when_no_duplicates() {
    let td = TempDir::new().unwrap();
    let root = td.path();
...
fn summary_alone_goes_to_stdout() {
    ...
    let out = fifi().arg("--summary").arg(root).assert().success();
    let stdout = String::from_utf8(out.get_output().stdout.clone()).unwrap();
